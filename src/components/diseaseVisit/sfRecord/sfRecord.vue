  ...
  <view class="sfRecord">
    ...
    <view class="content">
      <self-year-choose
        :curr-team="currTeam"
        :year-list="teamList"
        @yearChange="getMbInfo"
      ></self-year-choose>
      <view class="card-box">
        <view class="carditem" :class="{'active':curr=='高血压'}">
          <view v-if="sfRecord.length==0" class="notice">暂无记录！</view>
          <uni-swipe-action
            v-for="(item,index) in sfRecord"
            :key="index+'a'"
            :threshold="200"
            class="swiper-item"
          >
            <uni-swipe-action-item :right-options="options" @click="delSf(item.id)">
              <view class="card" @tap="gxySfHandle(item.id,index)">
                <view class="imgArea">
                  <img v-show="item.imgUrl" class="img" :src="$IMGURL + item.imgUrl" alt="" />
                </view>
                <view class="other">
                  <view class="data">
                    <text>{{item.data}}</text>
                    <text class="dw">mmHg</text>
                  </view>
                ...
                <view class="conclusion" :class="item.conclusion=='满意'?'okCss': ''">
                  {{item.conclusion}}
                </view>
              </view>
        ...
        <view class="carditem" :class="{'active':curr=='糖尿病'}">
          <view v-if="tnbList.length==0" class="notice">暂无记录！</view>
          <uni-swipe-action
            v-for="(item,index) in tnbList"
            :key="index"
            :threshold="200"
            class="swiper-item"
          >
            <uni-swipe-action-item :right-options="options" @click="delSf(item.id)">
              <view class="card" @tap="gxySfHandle(item.id,index)">
                <view class="imgArea">
                  <img v-show="item.imgUrl" class="img" :src="$IMGURL + item.imgUrl" mode="" />
                </view>
                <view class="other">
                  <view class="data">
                    <text>{{item.data}}</text>
                    <text class="dw">mmol/L</text>
                  </view>
                  <text class="classFlow">随访方式:{{getfolType(item.classifyFollowUp)}}</text>
                  <view class="status">
                    <view class="shStatus">
      ...
      <!-- 新增随访 -->
      <text class="iconfont icon-add-fill" @click="addSf()"></text>
    </view>
    <self-sign-pop
      v-model="toast"
      title="提示"
...
<script>
  ...
  export default {
  	...
  	created() {
  		// 年份处理
  		const nowYear = GetNowTime('.').substr(0, 4).toString()
  	const yearlist = [nowYear - 2, nowYear - 1, nowYear]
  	...
  	methods: {
  		...
  		okGoSign() {
  			let url = '/pages/home/doctorSign/doctorSign'
  			const post = this.goSignData
  		...
  		gxySfHandle(id, index) {
  			let url
  			const qureyData = JSON.parse(JSON.stringify(this.person))
  		qureyData.id = id

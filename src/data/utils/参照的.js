...
function deleteTable(tableName, callback) {
  ...
  plus.sqlite.executeSql({
    ...
    fail(e) {
      callback(
        myEntity.sqlRes('删除表 createChat', myEnum.status.ERROR, myEnum.sqliteException.ERROR, e)
      )
    },
  })
}
...
function insertManyMsg(tableName, msgs, callback) {
  ...
  let sql = 'INSERT INTO ' + tableName + ' (content,type,time,isMe) VALUES '
  sql =
    sql +
    '("' +
    msg[0].content +
